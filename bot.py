...
# ═══════════════════════════════════════════════════════════
# Settings — ready to run
# ═══════════════════════════════════════════════════════════

BOT_TOKEN = "8456156974:AAGHZQJn92FrU_tYJGpyqsFIo00Y28sEgXQ"
ADMIN_ID = 6363115803
...
BKASH_NUMBER = "01406595163"
BKASH_TYPE = "Personal"
BINANCE_ID = "906064151"
BINANCE_NOTE = "শুধু Binance Pay / UID তে পাঠান"
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == ST_TXN_AMOUNT:
        ...
        if amt is None:
            await update.message.reply_text("সঠিক পরিমাণ: 20 বা 50.50")
            return
        if amt < MIN_PAYMENT:
            await update.message.reply_text(
                f"মিনিমাম পেমেন্ট *{fmt_money(MIN_PAYMENT)} {CURRENCY}*",
                parse_mode="Markdown",
            )
            return
...
def main() -> None:
    ...
    if not token or "এখানে_" in token:
